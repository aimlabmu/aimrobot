model = dict(
    ...
    roi_head=dict(
        ...
        bbox_head=[
            dict(type='Shared2FCBBoxHead', num_classes=214),
            dict(type='Shared2FCBBoxHead', num_classes=214),
            dict(type='Shared2FCBBoxHead', num_classes=214)
        ]),
    train_cfg=dict(
...
data_root = '/workspace/fast_data_1/data/synth_cards/cards_hdr/'
img_norm_cfg = dict(
...
data = dict(
    ...
    train=dict(
        type='CocoDataset',
        ann_file=
        '/workspace/fast_data_1/data/synth_cards/cards_hdr/coco_labels.json',
        img_prefix='/workspace/fast_data_1/data/synth_cards/cards_hdr/images',
        pipeline=[
            dict(type='LoadImageFromFile'),
            dict(type='LoadAnnotations', with_bbox=True),
            dict(type='Resize', img_scale=(640, 640), keep_ratio=True),
        ...
        classes=(
            ...
            'takeapoo', 'washmyhair', 'washmyhand', 'person')),
    val=dict(
        type='CocoDataset',
        ann_file=
        '/workspace/fast_data_1/data/synth_cards/cardBox/robot_card_test_coco-human.json',
    ...
    test=dict(
        type='CocoDataset',
        ann_file=
        '/workspace/fast_data_1/data/synth_cards/cardBox2/label_pzaw.json',
        img_prefix='/workspace/fast_data_1/data/synth_cards/cardBox2',
        pipeline=[
            dict(type='LoadImageFromFile'),
            dict(
                type='MultiScaleFlipAug',
    ...
    hooks=[
        ...
        dict(
            ...
            init_kwargs=dict(
                project='mmdetection',
                entity='bpninja',
                config=dict(lr=0.01, batch_size=16),
                tags=['resnet50', 'sgd', 'cascade-rcnn', 'full-human']),
            interval=10,
            log_checkpoint=True,
...
classes = (
    ...
    'takeabath', 'takeapee', 'takeapoo', 'washmyhair', 'washmyhand', 'person')
fp16 = dict(loss_scale=512.0)
work_dir = './work_dirs/cascade_rcnn_r50_fpn_1x_human_card_detection'
auto_resume = False
gpu_ids = [0]
